...
export class NuevoComponent {
  ...
  guardarNotificacion() {
        ...
        if (!this.notificacion.titulo) {
          this._toasterService.pop('error', 'Ingrese el Fecha');
        } else if (!this.notificacion.tipo) {
          this._toasterService.pop('error', 'Seleccione observación');
        } else if (!this.notificacion.mensaje) {
          this._toasterService.pop('error', 'Ingrese descripción del feriado');
        }else{
